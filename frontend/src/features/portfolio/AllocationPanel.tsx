...
export function AllocationPanel({ snapshot, onUpdated }: Props) {
  ...
  return (
    <div className="portfolio-allocations" data-testid="portfolio-allocations">
      <h2>Allocations</h2>
      <p className="note">
        Allocations reserve capital under this portfolio. They do not start trading and
        are not strategy-owned wallets.
      </p>
          ...
          snapshot.allocations.map((allocation) => (
            ...
            >
              <div className="allocation-card__header">
                ...
                <span className="note" data-testid={`allocation-id-${allocation.id}`}>
                  id {allocation.id}
                </span>
              </div>
              ...
              <p className="note" data-testid={`allocation-activity-${allocation.id}`}>
                Performance: no activity (undeployed). Realized/unrealized P&amp;L: 0 USDT.
              </p>
              <p className="note">
                Parent portfolio available: {formatUsdt(snapshot.available)} (authoritative
                totals above; equity is not doubled by this reservation).
              </p>
