...
3. **Given** required configuration is incomplete or invalid (including `allocated_capital > starting_capital` or `max_position_size > allocated_capital`), **When** the user attempts to start, **Then** the system blocks start with a clear reason and does not begin trading activity.
...
- `allocated_capital > starting_capital` → reject create/start; do not begin the session.
- `max_position_size > allocated_capital` → reject create/start; do not begin the session.
...
- Starting simulated capital sets initial cash; **allocated capital** is a distinct enforceable deploy bound and MUST satisfy `allocated_capital ≤ starting_capital` at create/start (clarified 2026-08-09). **Maximum position size** MUST satisfy `max_position_size ≤ allocated_capital` (clarified 2026-08-09). UI MAY default allocated equal to starting when a single capital figure is entered, but both remain represented.
