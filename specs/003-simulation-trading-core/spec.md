...
1. **Given** strategy decisions have occurred on closed candles, **When** the user opens the Decision Journal, **Then** each closed-candle evaluation is listed, including HOLD outcomes as well as approved and rejected BUY/SELL outcomes with reasons when rejected.
...
- **FR-010**: Every closed-candle strategy evaluation MUST create a Decision Journal record for **HOLD**, **approved** non-HOLD, and **rejected** non-HOLD outcomes (including rejection reasons). Forced-close decisions SHOULD also be journaled when a forced flatten occurs.
...
- **Decision Journal Entry**: Trace record of each closed-candle strategy evaluation (HOLD, approved, or rejected) and forced-close decision traces when applicable.
...
- **SC-003**: After a run that includes at least one HOLD evaluation and at least one rejection, 100% of those observed closed-candle Decision Journal outcomes appear with correct outcome classification (and a reason when rejected).
...
- Journals are inspectable in-product for local single-operator use; multi-user auth is out of scope. Decision Journal includes HOLD as well as approve/reject on each closed-candle evaluation (clarified 2026-08-09).
