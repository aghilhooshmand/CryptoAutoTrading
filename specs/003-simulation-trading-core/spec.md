...
- Position model is long-only single full position (BUY only from flat; SELL only full close); clarified 2026-08-09. Full BUY notional is `min(cash/(1+fee_rate), max_position_size)` at the adverse fill price (plan).
...
- Mark-to-market equity and unrealized gross MAY be displayed; profit target / max loss use **liquidation** Session NET while LONG (hypothetical adverse SELL with fee/slippage). Hypothetical evaluation costs are not double-counted with the actual forced close (plan update 2026-08-09).
