...
- Mark-to-market for unrealized P&L uses the latest safe market price; if no safe price exists, unrealized valuation and new execution both fail safe rather than invent a price. Profit target / max loss use Session NET P&L including unrealized when equity is computable (plan).
