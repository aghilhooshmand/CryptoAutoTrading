...
- Q: When the operator manually stops a running session that still holds a long position, should the system force a simulated full close like other stops? → A: Yes — force full close when a safe price exists (fee/slippage, journal, forced-close marker); otherwise fail-safe unsafe-unflattened
- Q: Must allocated session capital be less than or equal to starting simulated capital when the operator configures a session? → A: Yes — require allocated_capital ≤ starting_capital at create/start; reject otherwise
- Q: Must every closed-candle strategy evaluation write a Decision Journal entry for HOLD, as well as for approved and rejected BUY/SELL outcomes? → A: Yes — journal HOLD, approved, and rejected for every closed-candle evaluation
- Q: **Amendment 2026-08-15 — Decision Log Mode**: Does every closed candle still require a durable HOLD Decision Journal row? → A: **Superseded.** Every closed candle is still processed. Durable persistence depends on effective `decision_log_mode`: `full_audit` = prior behavior (including HOLD); `important_only` (default for new sessions) does **not** persist ordinary HOLD (including warm-up HOLD). Approved/rejected/forced decisions and trade fills remain durable. Advancing `last_processed_candle_open_time` MUST NOT require writing a HOLD row. Legacy NULL mode = `full_audit`. No WebSocket/SSE/in-memory HOLD journal in this amendment.
- Q: Must maximum position size be less than or equal to allocated session capital when the operator configures a session? → A: Yes — require max_position_size ≤ allocated_capital at create/start; reject otherwise
...
3. **Given** required configuration is incomplete or invalid (including any violation of `0 < max_position_size ≤ allocated_capital ≤ starting_capital`), **When** the user attempts to start, **Then** the system blocks start with a clear reason and does not begin trading activity.
...
1. **Given** strategy decisions have occurred on closed candles and the session uses `full_audit`, **When** the user opens the Decision Journal, **Then** each closed-candle evaluation is listed, including HOLD outcomes as well as approved and rejected BUY/SELL outcomes with reasons when rejected.
2. **Given** a session uses `important_only`, **When** ordinary HOLD candles are processed, **Then** no durable HOLD Decision Journal row is created, `last_processed_candle_open_time` still advances, and the UI does not imply the session stopped processing solely because the journal is sparse; the journal indicates important-only logging.
3. **Given** approved, rejected, or forced decisions under either mode, **When** the user opens the Decision Journal, **Then** those durable rows are present with reasons when rejected/forced.
...
5. **Given** an active session, **When** the user stops the session manually, **Then** the session ends without requiring emergency stop, new signals do not execute afterward, and if a long is open the same forced-close / unsafe-unflattened rules as other stops apply.
...
- Full BUY would exceed `allocated_capital` or `max_position_size` (or affordable cash after fees) → reject; sizing uses `min(affordable_notional, allocated_capital, max_position_size)` with no partial silent oversize.
- Violation of `0 < max_position_size ≤ allocated_capital ≤ starting_capital` → reject create/start; do not begin the session.
...
- Same closed candle `openTime` already in `last_processed_candle_open_time` → do not evaluate again.
...
- Session ends with an open simulated position under manual stop, emergency stop, or automatic hard stop → if a safe price exists, force one simulated full close and journal it with a forced-close marker; if not, do not invent a close; stop new execution and keep the unsafe-unflattened state inspectable; never send a real exchange order.
...
- **FR-005**: A simulation session MUST be configurable with: trading pair, starting simulated capital, **allocated session capital**, one baseline strategy, strategy signal timeframe, session duration, **target net profit rate** (percent/fraction of allocated capital), **maximum session loss rate** (percent/fraction of allocated capital), maximum number of trades, maximum position size, and **`decision_log_mode`** (`important_only` \| `full_audit`). Sessions MUST NOT start without required capital/risk bounds defined. Capital sizes MUST satisfy `0 < max_position_size ≤ allocated_capital ≤ starting_capital` (reject create/start otherwise). The system MUST derive and persist absolute threshold amounts (`target_net_profit_amount = allocated_capital × target_net_profit_rate`, `max_session_loss_amount = allocated_capital × max_session_loss_rate`) together with the configured rates for auditability. Simulated fee and slippage rates MUST be overridable per session; when omitted, FR-012a defaults apply. Auto Trading MUST display both the configured percentage and the resulting currency amount for profit target and max loss. **`decision_log_mode`**: for **new** sessions, default **`important_only`** (from Settings default when seeded, or explicit create override); persist as effective configuration. Legacy sessions with NULL/missing mode MUST behave as **`full_audit`**. Changing Settings later MUST NOT alter an existing session’s `decision_log_mode`. Do not delete or rewrite existing decision rows on migration.
- **FR-005a**: Full-position BUY sizing MUST use:
  `affordable_notional = current_cash / (1 + fee_rate)` and
...
- **FR-006a**: The strategy MUST evaluate and emit a new signal **only when a candle for the session timeframe closes**. It MUST NOT emit trading signals from intrabar quote ticks or from still-forming (incomplete) candles. Between closed-candle evaluations, the prior signal does not re-trigger execution by itself. The session MUST persist `last_processed_candle_open_time` and MUST NOT evaluate the same closed candle `openTime` more than once. Advancing `last_processed_candle_open_time` MUST NOT depend on writing a Decision Journal row (a skipped HOLD under `important_only` is still a successfully processed candle).
...
- **FR-010**: **(Amended 2026-08-15 — Decision Log Mode.)** Every closed candle is still evaluated. Durable Decision Journal persistence depends on the session’s effective `decision_log_mode`:
  - **`full_audit`**: preserve original behavior — persist a Decision Journal record for every closed-candle evaluation including **HOLD**, plus **approved** and **rejected** non-HOLD outcomes (with rejection reasons), and forced-close decisions when a forced flatten occurs.
  - **`important_only`**: do **not** persist ordinary HOLD decisions (including warm-up HOLD). MUST still persist durable meaningful decisions: approved BUY/SELL; rejected BUY/SELL (Controller/Risk/execution) with reason code/message; forced decisions/actions. Trade fills remain in the Trade Journal. Session stop/emergency/failure information remains in existing authoritative session storage. Do not invent a new event table for this amendment.
  This supersedes the prior FR-010 rule that every candle MUST always produce a durable decision row.
- **FR-010a**: Live UI MUST NOT add WebSockets, SSE, or an in-memory HOLD journal for this amendment. Under `important_only`, the UI MUST NOT imply that lack of journal rows means the Simulation stopped processing; use existing session state / `last_processed_candle_open_time` (and related fields) and clearly indicate “Important decisions only” (or equivalent) when that mode is active.
...
- **FR-015a**: If a session stop ends the session while a long position is open—including automatic hard limits, emergency stop, and **manual stop**—and a **safe** market price is available, the system MUST execute one forced simulated full close (SELL of the entire long) at that price using documented fees/slippage, create a Trade Journal record with an explicit forced-close marker, then complete the stop. If no safe price is available, the system MUST NOT invent a close price: it MUST still stop new signal execution, fail safe, and leave the position marked as not safely flattened (inspectable), without fabricating P&L from a guessed exit.
...
- **Decision Journal Entry**: Durable trace of persisted decisions per effective `decision_log_mode` (under `full_audit`: each closed-candle evaluation including HOLD; under `important_only`: approved/rejected/forced, not ordinary HOLD).
...
- **SC-003**: After a `full_audit` run that includes at least one HOLD evaluation and at least one rejection, 100% of those closed-candle Decision Journal outcomes appear with correct classification (and a reason when rejected). After an `important_only` run that processes HOLD candles and at least one rejection, no ordinary HOLD rows are durable, the rejection is durable, and `last_processed_candle_open_time` advanced for skipped HOLDs.
...
- **SC-005**: When a session stop occurs—configured hard limits (Session NET P&L / trades / duration / unsafe-data), **manual stop**, or **emergency stop**—the session reaches a terminal stopped state and **0 further strategy-driven** simulated executions occur afterward in the observed test window. Exactly one forced safety close MAY still run during `STOPPING` when a long is open and a safe price exists (FR-015a); that forced fill is not a strategy-driven execution.
...
- Strategy evaluation runs only on each newly **closed** candle for the session timeframe; the session persists `last_processed_candle_open_time` and MUST NOT evaluate the same closed candle twice (FR-006a).
- Position model is long-only single full position (BUY only from flat; SELL only full close); clarified 2026-08-09. Full BUY notional uses `affordable_notional = current_cash/(1+fee_rate)` then `min(affordable_notional, allocated_capital, max_position_size)`.
...
- Capital invariant at create/start: `0 < max_position_size ≤ allocated_capital ≤ starting_capital` (clarified 2026-08-09). UI MAY default allocated equal to starting when a single capital figure is entered, but both remain represented.
...
- On hard stop, emergency stop, or **manual stop** with an open long: forced simulated full close if a safe price exists; otherwise fail-safe stop without inventing an exit (clarified 2026-08-09).
- Backend restart remains distinct: STOPPED with `backend_restart` and **no** auto-flatten (plan).
...
- Journals are inspectable in-product for local single-operator use; multi-user auth is out of scope. Decision Journal durability depends on effective `decision_log_mode` (amended 2026-08-15); prior 2026-08-09 HOLD-always clarify is superseded for `important_only`.
- Feature 004 Backtest decision persistence is **unchanged** by the Decision Log Mode amendment.
