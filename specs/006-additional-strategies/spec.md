...
- **FR-005**: The system MUST register Bollinger Bands as a strategy with canonical `strategy_id` = `bollinger_bands`, display name "Bollinger Bands", parameters: `period` (integer, default 20, min 2), `stdDev` (decimal_string, default `"2.0"`, exclusive minimum 0 — value MUST be strictly greater than 0). Catalog MAY expose `exclusiveMinimum: true` and/or constraint `std_dev_gt_zero`; it MUST NOT imply that `stdDev = 0` is valid.
