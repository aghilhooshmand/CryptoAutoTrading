...
- Q: Warm-up vs short history windows? → A: Fewer than **21** closed candles in the fetched window → fail with `insufficient_history` (no fabricated bars). Windows with ≥21 closed candles process normally; candles before Dual EMA warm-up readiness produce HOLD until the strategy is ready
- Q: When is a failed BacktestRun persisted? → A: Pre-run validation and oversized-history rejection create **no** run row. After a run is accepted into `running`, downstream fetch or execution failure persists a `failed` run (FIFO max 5 failed). Empty series and fewer than 21 closed candles after accept/fetch are `failed` / `insufficient_history`
...
- No historical candles available for the pair/timeframe/window → fail safely with `insufficient_history`; do not invent candles or prices.
- Fetched window contains fewer than **21** closed candles → fail with `insufficient_history`; do not invent bars or return empty “successful” results.
- Requested window would exceed the documented maximum candle count (or equivalent max span for that timeframe) → reject before run with a clear oversized-history reason; do not silently truncate or invent bars; do not create a BacktestRun row.
- Partial history (gaps) → do not fabricate missing bars; process only available closed candles in chronological order. For windows with ≥21 closed candles, early candles before Dual EMA warm-up readiness produce HOLD until the strategy is ready (no invented prior bars).
- Pre-run validation failures (invalid config, oversized history) → no durable BacktestRun row.
- After a run has been accepted into `running`, fetch or execution failure → persist `status=failed` (failed FIFO max 5).
...
- **FR-003**: Backtesting MUST enforce the pipeline: historical Market Data → Strategy Engine → Trading Signal → Trading Controller → Risk Manager → Historical Execution (HistoricalExecutionAdapter) → Position/Balance → continue or stop. Strategies MUST NOT modify balances or positions directly.
...
- **FR-007**: The Dual EMA strategy implementation used by Feature 003 simulation MUST be reusable by backtesting; Feature 004 MUST NOT ship a separate duplicate Dual EMA implementation. (Broader shared-module reuse is FR-013.)
...
- **FR-012**: Historical execution in backtests MUST use a **HistoricalExecutionAdapter** (backtest-specific) that applies the same fee and adverse-slippage model as Feature 003 (defaults 0.10% fee and 0.05% adverse slippage per side unless overridden) and MUST update balances/positions deterministically with precise money arithmetic consistent with Feature 003 (no imprecise floating-point money math). Live simulation execution MUST NOT be reused for historical fill timing.
- **FR-013**: Strategy, controller, risk, accounting, and sizing MUST reuse shared Feature 003 domain logic where clean reuse is possible rather than forking incompatible copies (extends FR-007’s Dual EMA reuse to the broader control/accounting stack). Historical fill timing remains isolated in the backtest execution adapter.
...
- **FR-018b**: Failed runs that were persisted after an accepted start MUST be retained at most **5** (FIFO oldest); failed retention is separate from the 20 completed quota. Pre-run validation failures and oversized-history rejection MUST NOT create a BacktestRun row and MUST NOT consume failed retention. Once a run has been accepted and entered `running`, downstream fetch or execution failure MUST persist a `failed` run.
...
- **SC-006**: When historical data for the requested window is missing (empty series) or has fewer than 21 closed candles, 100% of observed attempts fail safely with `insufficient_history` and 0 fabricated candles/prices used for fills.
...
- Session “duration” as a live wall-clock bound is not a primary backtest input; the historical start/end window defines the evaluation period (constitution V historical-backtest exception).
...
- Fetched windows with fewer than **21** closed candles fail with `insufficient_history`; longer windows HOLD through Dual EMA warm-up until ready.
- Pre-run validation / oversized rejection creates no BacktestRun row; post-accept fetch/execution failures persist `failed` (FIFO 5).
