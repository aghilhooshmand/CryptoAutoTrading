...
1. **Given** the application is running locally without exchange trading credentials, **When** the operator configures a valid backtest (pair, timeframe, start/end, capital nesting, fee/slippage) and runs it, **Then** the system completes one backtest run and returns a result for that configuration.
2. **Given** required inputs are missing or invalid (including violation of `0 < max_position_size ≤ allocated_capital ≤ starting_capital`, end before start, unsupported timeframe, or unsupported pair), **When** the operator attempts to run, **Then** the system blocks the run with a clear reason and does not invent results.
...
2. **Given** a non-HOLD signal valid for the long-only full-position model, **When** control and risk approve it, **Then** a simulated BUY opens a full long from flat, or a simulated SELL fully closes the long, using historical candle pricing context plus documented fee/slippage—never a real exchange order.
...
- Backtest ends while still long → flatten once at the last processed closed candle using the same fee/slippage model so ending capital and net P&L are determinate (mark this flatten as end-of-run, not a strategy signal).
...
- **FR-015**: If the backtest is still long after the last processed candle in range (or after an early exit bound), the system MUST perform exactly one end-of-run full simulated close using that candle’s pricing context and the session fee/slippage assumptions so ending capital is determinate; this flatten MUST be inspectable and distinguished from ordinary strategy fills.
...
- **FR-017**: Buy-and-hold return MUST be computed for the same pair and backtest window as a comparison baseline using one synthetic full entry near the first usable closed candle in range and one full exit at the last processed closed candle, applying the same fee and adverse-slippage assumptions once on entry and once on exit so the comparison is cost-aware.
...
- **FR-019**: Maximum drawdown MUST be measurable from the equity path of the backtest (using net/liquidation-consistent equity after costs where positions are open) and reported in the summary.
...
- **Backtest Run**: One bounded historical evaluation with pair, timeframe, window, capital nesting, optional profit/loss rates, fee/slippage, status (e.g. configured/running/completed/failed), and links to results.
- **Backtest Configuration**: Operator inputs that fully determine a run when combined with a fixed historical candle series.
...
- **Backtest Trade**: Deterministic simulated fill (strategy or end-of-run flatten) with fees/slippage; never a real exchange order.
- **Backtest Decision Record**: Trace of each processed closed-candle evaluation (HOLD / approved / rejected).
- **Backtest Summary**: Aggregate performance metrics required by FR-016, including buy-and-hold comparison and maximum drawdown.
- **Equity Point**: Point on the run’s equity path used to compute drawdown and ending capital.
...
- Maximum trades may be enforced by risk consistently with Feature 003 when configured; if not supplied in v1 UI, an explicit high default or omission with documented behavior is acceptable so long as determinism and capital nesting remain intact.
- End-of-run open longs are always flattened once for determinate ending capital.
- Buy-and-hold comparison uses one cost-aware entry and one cost-aware exit over the same window (FR-017), not an infinite compounding model.
